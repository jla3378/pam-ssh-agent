...
pub fn read_message(input: &mut dyn Read) -> Result<ReadMessage> {
    ...
    match t {
        ...
        SSH_AGENT_SIGN_RESPONSE => {
            ...
            if !buf.is_empty() {
                return invalid_data("trailing signature data");
            }
...
fn make_identities<'a>(mut buf: Bytes) -> Result<Vec<Identity<'a>>> {
    ...
    if !buf.is_empty() {
        return invalid_data("trailing identity data");
    }
    Ok(result)
}
...
mod test {
    ...
    #[test]
    fn test_read_message_rejects_trailing_identity_data() {
        let mut cursor = reader(b"\0\0\0\x06\x0c\0\0\0\0\0");
        assert!(matches!(
            read_message(&mut cursor),
            Err(Error::InvalidMessage(message)) if message == "trailing identity data"
        ));
    }

    #[test]
    fn test_read_message_rejects_trailing_signature_data() {
        let mut message = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/tests/data/sign_response.bin"
        ))
        .to_vec();
        let length = u32::from_be_bytes(message[..4].try_into().unwrap());
        message[..4].copy_from_slice(&(length + 1).to_be_bytes());
        let signature_length = u32::from_be_bytes(message[5..9].try_into().unwrap());
        message[5..9].copy_from_slice(&(signature_length + 1).to_be_bytes());
        message.push(0);

        let mut cursor = Cursor::new(message);
        assert!(matches!(
            read_message(&mut cursor),
            Err(Error::InvalidMessage(message)) if message == "trailing signature data"
        ));
    }
