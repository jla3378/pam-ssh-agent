...
mod test {
    ...
    fn fuzz_validate_cert() {
        ...
        for _ in 0..fuzz_iters() {
            let s = f.next_string();
            // Clamp to a representable SystemTime (production passes SystemTime::now()).
            let when = SystemTime::UNIX_EPOCH
                .checked_add(Duration::from_secs(f.any_u64()))
                .unwrap_or(SystemTime::UNIX_EPOCH);
            let principal = f.next_string();
            let probe = s.clone();
            assert_no_panic("from_openssh/validate_cert", probe, || {
